...
export function initIcons(iconsProvider: IconsProvider): () => Promise<boolean> {
    return () => iconsProvider.init();
}

// existing icons: https://material.io/tools/icons/?style=baseline
// https://thenounproject.com/search/?q=key&i=1436196
//
@Injectable()
export class IconsProvider {
    ...
    public init(): Promise<boolean> {
        ...
        return new Promise<boolean>((resolve) => {
            this.iconRegistry
            // use like this:   <mat-icon svgIcon="bike"></mat-icon>
                .addSvgIconLiteral('bike',
                    this.sanitizer.bypassSecurityTrustHtml(BIKE_ICON))
            //
            // .addSvgIconLiteral('key',
            //     this.sanitizer.bypassSecurityTrustHtml(KEY_ICON))
            // .addSvgIconSetLiteralInNamespace('core-inline',
            //     this.sanitizer.bypassSecurityTrustHtml(INLINE_ICON_SET))
            // .registerFontClassAlias('fontawesome', 'fa')
            // .addSvgIconSetInNamespace('core',
            //     this.sanitizer.bypassSecurityTrustResourceUrl('/icon/assets/core-icon-set.svg'))
            // .addSvgIcon('thumb-up',
            //     this.sanitizer.bypassSecurityTrustResourceUrl('/icon/assets/thumbup-icon.svg'))
            ;
